...
module "fe_s3" {
  source                                    = "../../modules/s3"
  bucket_name                               = var.fe_certificate_domain_name
  cloudfront_origin_access_identity_iam_arn = module.fe_origin_access_identity.cloudfront_access_identity_iam_arn
}

module "fe_origin_access_identity" {
  source                         = "../../modules/cloudfront/origin_access_identity"
  origin_access_identity_comment = var.bucket
}

module "fe_cloudfront_cert" {
  source                  = "../../modules/certificate"
  route53_hosted_zone     = var.route53_hosted_zone
  route53_zone_id         = data.aws_route53_zone.selected.id
  certificate_domain_name = var.fe_certificate_domain_name
...
module "fe_cloudfront" {
  source                          = "../../modules/cloudfront"
  bucket_regional_domain_name     = module.fe_s3.bucket_regional_domain_name
  alternate_domain_names          = [var.fe_certificate_domain_name]
  is_web_static_page              = true
  cloudfront_access_identity_path = module.fe_origin_access_identity.cloudfront_access_identity_path
  acm_certificate_arn             = module.fe_cloudfront_cert.acm_certificate_arn
  ssl_support_method              = var.ssl_support_method
}

module "fe_route53" {
  source          = "../../modules/route53"
  domain_name     = var.fe_certificate_domain_name
  route53_zone_id = data.aws_route53_zone.selected.id
  alias_name      = module.fe_cloudfront.domain_name
  alias_zone_id   = module.fe_cloudfront.hosted_zone_id
}
